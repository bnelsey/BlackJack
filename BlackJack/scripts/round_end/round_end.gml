...
function round_end(){
	...
	if (dealer_value > player_value and dealer_value <= 21) or player_value > 21
	{
	...
	if dealer_value == player_value
	{
		...
		obj_coin.change_player_balance = bet_value		
		obj_coin.destroy_after_anim = true
		bet_value = 0
		obj_coin.alarm[0] = 1 // start moving
		alarm[4] = 30
	...
	else // player win
	{
		...
		// move chip
		_new_chip.targetx = bet_x
		_new_chip.targety = bet_y				
		_new_chip.alarm[0] = 1
		_new_chip.destroy_after_anim = true
		
		if player_value == 21
			_new_chip.change_player_bet = (bet_value/2) * 3
		else
			_new_chip.change_player_bet = bet_value
		...
		_new_chip.destroy_after_anim = true
		if player_value == 21
			_new_chip.change_player_balance = bet_value + ((bet_value/2) * 3)
		else
			_new_chip.change_player_balance = bet_value*2
